...
const Register = () => {
  ...
  return (
    <AnimatedSection className="section container" style={{ maxWidth: 460 }}>
      ...
      <AnimatePresence mode="wait">
        ...
        {stage === "otp" && (
          ...
          >
            ...
            <motion.button
              type="submit"
              className="btn btn-marigold w-100 mb-2 d-flex align-items-center justify-content-center gap-2"
              disabled={submitting}
              {...buttonBounce}
            >
              {submitting ? (<><FaSpinner className="spin-icon" /> {t("auth.verifying")}</>) : t("auth.verifyButton")}
            </motion.button>
            <motion.button type="button" className="btn btn-outline-maroon w-100" onClick={handleResend} {...buttonBounce}>
              {t("auth.resendCode")}
            </motion.button>
          </motion.form>
        )}
